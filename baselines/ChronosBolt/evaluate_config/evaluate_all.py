...
if __name__ == '__main__':
    ...
    # write logs to file
    with open('chronos_bolt_evaluation_logs.txt', 'w') as f:
        for log in logs:
            f.write(log)
            f.write('\n' + '='*80 + '\n')

    # write eval_result to file
    with open('chronos_bolt_evaluation_results.txt', 'w') as f:
        for key, value in eval_result.items():
            f.write(f'{key}: MAE={value["MAE"]}, MSE={value["MSE"]}\n')
    
    with open('chronos_bolt_evaluation_results.json', 'w') as f:
        import json
