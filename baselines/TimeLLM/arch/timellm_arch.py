...
class TimeLLM(nn.Module):
    ...
    def __init__(self, **model_args):
        ...
        self.patch_embedding = PatchEmbedding(
            model_args['d_model'], self.patch_len, self.stride, model_args['dropout'])
        ...
        self.reprogramming_layer = ReprogrammingLayer(model_args['d_model'], model_args['n_heads'], self.d_ff, self.d_llm)

        self.patch_nums = int((self.seq_len - self.patch_len) / self.stride + 2)
        self.head_nf = self.d_ff * self.patch_nums

        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            self.output_projection = FlattenHead(model_args['enc_in'], self.head_nf, self.pred_len, head_dropout=model_args['dropout'])
        else:
            raise NotImplementedError

        self.normalize_layers = Normalize(model_args['enc_in'], affine=False)
