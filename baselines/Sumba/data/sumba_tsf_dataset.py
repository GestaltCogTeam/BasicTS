...
class SumbaTimeSeriesForecastingDataset(TimeSeriesForecastingDataset):
    """
    ...
    def __init__(self, dataset_name: str, train_val_test_ratio: List[float], mode: str, input_len: int, output_len: int,  \
        embed: str = 'timeF', freq: str = 'h', overlap: bool = False, logger: logging.Logger = None) -> None:
        """
        ...
        self.time_file_path = f'datasets/{dataset_name}/time.npy'
        self.embed = embed
        self.freq = freq
    ...
    def _load_time(self) -> np.ndarray:
        ...
        try:
            data = np.load(self.time_file_path)
        except (FileNotFoundError, ValueError) as e:
            raise ValueError(f'Error loading data file: {self.time_file_path}') from e
        ...
        if timeenc == 0:
            ...
            df['hour'] = data.hour
            df['minute'] = data.minute // 15
            data = df.values
        elif timeenc == 1:
            data = time_features(data, freq=self.freq)
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        history_t_index = self.time[index:index + self.input_len]
        future_t_index = self.time[index + self.input_len:index + self.input_len + self.output_len]
        return {'inputs': history_data, 'target': future_data, 'inputs_t_index': history_t_index, 'target_t_index': future_t_index}
