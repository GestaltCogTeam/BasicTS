...
class DGCRN(nn.Module):
    ...
    def step(self, input, Hidden_State, Cell_State, predefined_A, type='encoder', i=None):
        ...
        if type == 'encoder':
            filter1 = self.GCN1_tg(hyper_input, predefined_A[0]) + self.GCN1_tg_1( hyper_input, predefined_A[1])
            filter2 = self.GCN2_tg(hyper_input, predefined_A[0]) + self.GCN2_tg_1( hyper_input, predefined_A[1])

        if type == 'decoder':
            filter1 = self.GCN1_tg_de(hyper_input, predefined_A[0]) + self.GCN1_tg_de_1( hyper_input, predefined_A[1])
            filter2 = self.GCN2_tg_de(hyper_input, predefined_A[0]) + self.GCN2_tg_de_1( hyper_input, predefined_A[1])
    ...
    def forward(self, history_data:torch.Tensor, future_data:torch.Tensor=None, batch_seen:int=None, task_level:int=12, **kwargs) -> torch.Tensor:
        ...
        Args:
            ...
            batches_seen (int, optional): batch num. Defaults to None.
        ...
        batch_size = x.size(0)
        Hidden_State, Cell_State = self.initHidden(batch_size * self.num_nodes, self.hidden_size)
        ...
        go_symbol = torch.zeros((batch_size, self.output_dim, self.num_nodes)).to(input.device)
        timeofday = ycl[:, [1], :, :]
        ...
        for i in range(task_level):
            try:
                ...
                sys.exit(0)
            Hidden_State, Cell_State = self.step(decoder_input, Hidden_State, Cell_State, predefined_A, 'decoder', None)
