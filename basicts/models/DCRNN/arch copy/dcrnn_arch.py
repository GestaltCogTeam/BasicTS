...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, **model_kwargs):
        ...
        layer_0 = DCGRUCell(
            self.input_dim, self.rnn_units, adj_mx, self.max_diffusion_step, self.num_nodes)
        self.dcgru_layers = nn.ModuleList([layer_0])
        self.dcgru_layers.extend(
            [DCGRUCell(self.rnn_units, self.rnn_units, adj_mx, self.max_diffusion_step, self.num_nodes) for _ in range(1, self.num_rnn_layers)])
...
class DecoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, **model_kwargs):
        ...
        layer_0 = DCGRUCell(
            self.output_dim, self.rnn_units, adj_mx, self.max_diffusion_step, self.num_nodes)
        self.dcgru_layers = nn.ModuleList([layer_0])
        self.dcgru_layers.extend(
            [DCGRUCell(self.rnn_units, self.rnn_units, adj_mx, self.max_diffusion_step, self.num_nodes) for _ in range(1, self.num_rnn_layers)])
