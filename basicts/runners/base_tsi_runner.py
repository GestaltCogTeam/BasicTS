...
class BaseTimeSeriesImputationRunner(BaseEpochRunner):
    """
    Runner for multivariate time series imputation tasks.
    ...
    def __init__(self, cfg: Dict):
        ...
        # mask ratio
        self.mask_ratio = cfg.get('MASK_RATIO', 0.0)
    ...
    def on_train_start(self, cfg: Dict):
    ...
    def _init_validation(self, cfg: Dict):
    ...
    def forward(self, data: torch.Tensor, mask: torch.Tensor, epoch: Optional[int] = None, iter_num: Optional[int] = None, train: bool = True, **kwargs) -> Dict:
        """
        Performs the forward pass for training, validation, and testing. 
        Note: The outputs are not re-scaled.

        Args:
            data (torch.Tensor): A dictionary containing 'target' (future data) and 'inputs' (history data) (normalized by self.scaler).
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        masked_inputs, mask = self._mask(data)

        forward_return = self.forward(data=masked_inputs, mask=mask, epoch=epoch, iter_num=iter_num, train=True)

        loss = self.metric_forward(self.loss, forward_return)
        weight = self._get_metric_weight(forward_return['target'])
        self.update_meter('train/loss', loss.item(), weight)

        for metric_name, metric_func in self.metrics.items():
            metric_item = self.metric_forward(metric_func, forward_return)
            self.update_meter(f'train/{metric_name}', metric_item.item(), weight)
        return loss
    ...
    def val_iters(self, iter_index: int, data: Union[torch.Tensor, Tuple]):
        ...
        loss = self.metric_forward(self.loss, forward_return)
        weight = self._get_metric_weight(forward_return['target'])
        self.update_meter('val/loss', loss.item(), weight)

        for metric_name, metric_func in self.metrics.items():
            metric_item = self.metric_forward(metric_func, forward_return)
            self.update_meter(f'val/{metric_name}', metric_item.item(), weight)
    ...
    def test(self, train_epoch: Optional[int] = None, save_metrics: bool = False, save_results: bool = False) -> Dict:
        ...
        for batch_idx, data in enumerate(tqdm(self.test_data_loader)):
            ...
            loss = self.metric_forward(self.loss, forward_return)
            weight = self._get_metric_weight(forward_return['target'])
            self.update_meter('test/loss', loss.item(), weight)
            ...
            for metric_name, metric_func in self.metrics.items():
                ...
                self.update_meter(f'test/{metric_name}', metric_item.item(), weight)
    ...
    def _mask(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        
        """
        Mask the input tensor.

        Args:
            inputs (torch.Tensor): The input tensor to be masked.

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: A tuple containing the masked input tensor and the mask tensor.
        """

        mask = torch.rand_like(inputs) > self.mask_ratio # 0 for masked, 1 for remained
