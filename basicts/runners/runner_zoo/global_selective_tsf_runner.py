...
class GlobalSelectiveTimeSeriesForecastingRunner(BaseTimeSeriesForecastingRunner):
    """
    ...
    def __init__(self, cfg: Dict):
        ...
        if self.save_loss: 
            ...
            self.history_loss = np.load('/home/fys/BasicTS/history_loss/ETTh1_336_336/history_loss_100.npy',allow_pickle=True)
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        res: torch.Tensor = forward_return['prediction'] - forward_return['target']
        ...
        if self.selective_weights is not None:
            forward_return["selective_weights"] = self.selective_weights[expanded_idx, :].unsqueeze(-1)
    ...
    def on_epoch_end(self, epoch: int) -> None:
        ...
        # gap = torch.tensor((np.abs(self.current_loss) - self.history_loss) / self.history_loss) # [N, H, C]
        # gap = self.to_running_device(gap)
        # gap = gap.reshape(-1, self.num_nodes) # [N * H, C]
        # mask_ratio = 0.2
        # thresholds = torch.quantile(
        #     gap, mask_ratio, dim=0, keepdim=True
        # )
        # res_mask = gap < thresholds
        # self.selective_weights = torch.ones(gap.shape, dtype=torch.float32)
        # self.selective_weights[res_mask] = 0.0
        # self.selective_weights = self.selective_weights.reshape(-1, self.output_len, self.num_nodes)
        ...
        mask_ratio = 0.1
        thresholds = torch.quantile(
            coe_var, 1 - mask_ratio, dim=0, keepdim=True
        )
        res_mask = coe_var >= thresholds
        self.selective_weights = torch.ones(coe_var.shape, dtype=torch.float32)
        self.selective_weights[res_mask] = 0.0
