...
from .utils import InfiniteGenerator
...
class BaseIterationRunner(metaclass=ABCMeta):
    ...
    Other Features:
        ...
        - Support AdamWFused and Motu Optimizer from nanoGPT (TODO)
    ...
    def load_model_resume(self, strict: bool = True) -> None:
        ...
        try:
            ...
            if self.scheduler is not None:
                self.scheduler.last_epoch = checkpoint_dict['iteration']
    ...
    def init_validation(self, cfg: Dict) -> None:
        ...
        assert self.val_interval and self.val_interval > 0 and self.val_interval <= self.num_iterations, \
            'val_interval must be in range (0, num_iterations]'
    ...
    def on_iteration_end(self, iteration: int) -> None:
        ...
        self.plt_iteration_meters('train', iteration)
        if iteration % self.val_interval == 0:
            # print train meters
            self.print_iteration_meters('train')
            # validate
            if self.val_data_loader is not None and iteration % self.val_interval == 0:
    ...
    @master_only
    def plt_iteration_meters(self, meter_type, step) -> None:
