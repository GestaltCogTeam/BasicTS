...
@dataclass
class BasicTSClassificationConfig(BasicTSConfig):

    """
    BasicTS Classification Config, including general configuration, dataset and scaler configuration, model configuration, \
    metrics configuration, training configuration, validation configuration, test configuration, evaluation configuration, \
    and environment configuration.
    
    **Required Fields:** These fields must be specified for running BasicTS classification tasks.
    ...
    **BasicTSObject parameters:** You can two ways to construct a BasicTSObject () except for model.
    ...
    taskflow: BasicTSTaskFlow = field(default=BasicTSClassificationTaskFlow(),
    ...
    dataset_params: dict | None = field(default=None, metadata={"help": "Dataset parameters."})
    ...
    metrics: List[str] = field(
        default_factory=lambda: ["Accuracy"], metadata={"help": "Metric names."})
    ...
    loss: Callable = field(default=CrossEntropyLoss(), metadata={"help": "Loss function."})
    ...
    lr_scheduler: type | None = field(default=None, metadata={"help": "Learning rate scheduler type."})
    lr_scheduler_params: dict | None = field(default=None, metadata={"help": "Learning rate scheduler parameters."})

    # Checkpoint loading and saving settings
    ckpt_save_dir: str = field(
        default=None,
        metadata={"help": "Directory to save checkpoints." \
                  "Default: 'checkpoints/{model_name}/{dataset_name}_{num_epochs}', which will be post-initialized."})
    ...
    train_data_collate_fn: Callable | None = field(
    ...
    val_data_collate_fn: Callable | None = field(
    ...
    test_data_collate_fn: Callable | None = field(
    ...
    def __post_init__(self):
        ...
        if self.ckpt_save_dir is None:
            self.ckpt_save_dir = \
                f"checkpoints/{self.model.__name__}/{self.dataset_name}_{self.num_epochs}"
